...
const getdata=async()=>{
  let result=await fetch("https://restaurant-backend-4w5p.onrender.com/comments")
...
let deleteComment=async(id)=>{
      console.log(id)
      let result = await fetch(`https://restaurant-backend-4w5p.onrender.com/comment/${id}`, {
        method: "Delete",
      });
      result = await result.json();
      if (result) {
