...
function Reserve() {
  ...
  let clickHandler=async()=>{
    ...
    let result=await fetch("https://restaurant-backend-4w5p.onrender.com/reserve",{
      method:'post',
...
console.log(result)
 ...
 navigate("/")
  ...
  let result = await fetch(`https://restaurant-backend-4w5p.onrender.com/order/${id}`, {
    method: "Delete",
  });
  result = await result.json();
  if (result) {
  ...
  <div className="col-md-4">
    ...
    <input type="text" className="form-control" id="validationDefault01" placeholder="Srijan Bhardwaj" value={name} onChange={(e)=>setName(e.target.value)}/>
  ...
  <div className="col-md-4">
    ...
    <div className="input-group">
      <input type="email" className="form-control" id="validationDefaultUsername"  aria-describedby="inputGroupPrepend2" placeholder="srijan@gmail.com" value={email} onChange={(e)=>setEmail(e.target.value)} />
    </div>
  </div>
  {/* people */}
  <div className="col-md-4">
